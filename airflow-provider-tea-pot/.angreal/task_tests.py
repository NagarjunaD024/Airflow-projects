...
def run_tests(integration=False,full=False,open=False):
    ...
    if integration: 
        subprocess.run(f"{venv_python} -mpytest -vvv --cov=tea_pot --cov-report html --cov-report term tests/integration",shell=True, cwd=cwd)
    if full:
        subprocess.run(f"{venv_python} -m pytest -vvv --cov=tea_pot --cov-report html --cov-report term tests/",shell=True, cwd=cwd)
    if not integration and not full:
        subprocess.run(f"{venv_python} -m pytest -vvv --cov=tea_pot --cov-report html --cov-report term tests/unit",shell=True, cwd=cwd)    
    if open:
...
def static(open):
    subprocess.run(
        (
            f"{venv_python} -m mypy tea_pot --ignore-missing-imports --html-report typing_report"
        ),
        shell=True,
        cwd=cwd
    )
